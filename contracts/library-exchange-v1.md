...
The `backup-runtime` module exposes `com.dongholab.pagetuner.core.backup.exchange.LibraryExchangeCodec.read(ByteArray): LibraryExchangePackage` and `write(LibraryExchangePackage): ByteArray`. Framework-free data classes and format limits remain in `core-backup`, in `LibraryExchangeModels.kt`; `ExchangeAsset(bytes, mimeType)` derives `sha256` and `path`. `ExchangeDocument.extensionsJson` is encoded as the wire `extensions` JSON object. Android and server depend on `backup-runtime` and provide the JSON runtime; platform dependencies must not enter the core model module.
