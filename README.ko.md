...
- 로컬 서재는 폴더별로 책을 묶어 보여주고, 각 책 row에서 폴더/태그를 저장할
  수 있습니다.
- 로컬 서재 검색은 제목, 형식, 폴더, 태그를 대상으로 동작하며, 폴더 chip으로
  카테고리처럼 빠르게 필터링할 수 있습니다.
...
- 컬러 서비스와 흑백 서비스를 위한 앱 전역 display palette.
- PDF 렌더링은 선택한 Display 모드를 따릅니다.
- PDF용 공통 bitmap display transform을 EPUB/커버/썸네일 이미지 파이프라인
  기반으로 분리했습니다.
- 수동 전자잉크 새로고침 액션은 렌더링된 쪽 캐시를 비우고 현재 쪽을 다시
  렌더링합니다.
- PDF 쪽 맞춤 / 너비 맞춤 설정.
...
- 한국어/영어 번역 프리셋:
  ...
  - 한국어 → 영어
  - 자동 감지 → 영어
- 시작언어와 종료언어를 각각 언어 칩으로 지정할 수 있고, 직접 언어 코드를
  입력하는 방식도 유지합니다.
