...
export function ReaderTools({ namespace, document, anchor, onJump, onClose }: {
  ...
  const [tab, setTab] = useState<'bookmark' | 'note' | 'highlight' | 'add' | 'outline' | 'images' | 'search' | 'settings' | 'export'>('bookmark')
  ...
  useEffect(() => {
    let active = true
    storage.list(document).then(value => { if (active) { setItems(value.items); setDamagedIds(value.damagedIds) } })
      .catch(error => { if (active) setError(deviceStorageMessage(error)) })
    return () => { active = false }
  }, [document, storage])
  async function write(action: () => Promise<unknown>) {
    setBusy(true); setError('')
    try { await action(); await refresh(); setRemoveId(undefined) } catch (error) { setError(deviceStorageMessage(error)) } finally { setBusy(false) }
  }
  const labels = { bookmark: '북마크', note: '메모', highlight: '강조', add: '현재 위치에 추가', outline: '목차', images: '삽화', search: '본문 검색', settings: '독서 설정', export: '내보내기 · 공유' }
  ...
  if (selected) return <PagedReader document={{ id: `reading-note:${selected.id}`, bookTitle: document.bookTitle, chapterTitle: selected.title, language: document.language, kind: 'introduction',
    paragraphs: [{ paragraphId: 'note-body', text: selected.text || selected.excerpt }] }} onClose={() => setSelected(undefined)} onAnchorChange={() => {}} positionNote={t('이 기기에 저장한 읽기 기록입니다.')}/>
  ...
  return <section className="reading-workspace" aria-label={t('읽기 도구')}>
    ...
    <nav className="workflow-subtabs" aria-label={t('읽기 도구')}>
      {(Object.keys(labels) as Array<keyof typeof labels>).filter(value => (value !== 'outline' || document.outline?.length) && (value !== 'images' || document.assets?.images?.length) && (value !== 'highlight' || document.local?.format !== 'pdf')).map(value =>
        <button key={value} aria-pressed={tab === value} onClick={() => { setTab(value); setRemoveId(undefined) }} disabled={busy}>{t(labels[value])}</button>)}
    </nav>
    ...
    {removeId ? <div className="reading-confirm"><h2>{t('이 기기에서 항목을 삭제할까요?')}</h2><p>{t('삭제한 북마크와 메모는 복구할 수 없습니다.')}</p><div>
      <button className="button-outline" disabled={busy} onClick={() => setRemoveId(undefined)}>{t('취소')}</button>
      <button className="button-primary" disabled={busy} onClick={() => void write(() => storage.remove(document.id, removeId))}>{t('삭제')}</button></div></div>
    ...
    : <>{damagedIds.length > 0 && <div role="alert" className="workflow-message">{t('손상된 읽기 기록 {0}개가 있습니다.', [damagedIds.length])} <button className="button-text" onClick={() => setRemoveId(damagedIds[0])}>{t('손상된 항목 삭제')}</button></div>}
      {rows.length ? <AdaptiveCollection items={rows} itemKey={item => item.id} rowHeight={132} renderItem={item => <div className="reading-note-item"><div className="reading-note-copy"><button className="reading-note-title" onClick={() => setSelected(item)}>{item.title}</button><p>{item.kind === 'note' ? item.text : item.excerpt}</p></div><div className="reading-row-actions"><button className="button-outline" onClick={() => onJump(item.anchor)}>{t('이동')}</button><button className="button-quiet" onClick={() => setRemoveId(item.id)} disabled={busy}>{t('삭제')}</button></div></div>}/>
    ...
    <p className="reading-tools-caption">{t('읽기 기록은 이 계정 이름으로 이 기기에만 저장됩니다.')}</p>
  </section>
}
