    ...
    override suspend fun translate(request: TranslationRequest): List<TranslatedSegment> {
        ...
        return withContext(Dispatchers.IO) {
            runCatching {
                ...
                }.also { validateTranslationResponse(request.segments, it, ProviderName) }
            }.getOrElse { error ->
        ...
        return runCatching {
            ...
            buildString {
                for (index in 0 until sentences.length()) {
                    val sentence = sentences.optJSONArray(index)
                        ?: throw IOException("Response contained an invalid sentence fragment.")
                    append(sentence.opt(0) as? String
                        ?: throw IOException("Response contained a missing sentence translation."))
                }
...
object GoogleWebTranslateHtmlResponseParser {
    ...
    fun parse(response: String, expectedCount: Int): Result<List<String>> {
        ...
        val joined = allStrings.joinToString("")
        if (joined.contains(Regex("<a\\s", RegexOption.IGNORE_CASE))) {
            return runCatching { parseAnchoredTranslations(joined, expectedCount) }
        }

        val directCandidate = findDirectStringArrayCandidate(root, expectedCount)
        if (directCandidate != null) {
            return Result.success(directCandidate.map { it.cleanupTranslatedHtml() })
        }
    ...
    private fun parseAnchoredTranslations(
        ...
        val ids = matches.map { it.groupValues[1].toIntOrNull() }
        if (matches.size != expectedCount || ids.toSet() != (0 until expectedCount).toSet()) {
            throw IOException("Response contained duplicate, missing or unexpected segment IDs.")
        }
        val byIndex = matches.associate { it.groupValues[1].toInt() to it.groupValues[2].cleanupTranslatedHtml() }
        return (0 until expectedCount).map { byIndex.getValue(it) }
    }
