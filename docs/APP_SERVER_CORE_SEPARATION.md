...
2026-09-15 ZIP 교환 모델·제한은 순수 `core-backup/`, JSON·ZIP codec은 공용
`backup-runtime/`으로 분리했습니다. 앱과 서버는 같은 codec을 소비합니다.
...
flowchart TD
    ...
    APP --> BACKUP_RUNTIME["backup-runtime · 표준 ZIP 검증/입출력"]
...
공용 runtime은 Kotlin/JVM 실행 라이브러리입니다. HTTP·JSON·코루틴을 사용할 수 있지만
...
./gradlew -PbuildTarget=core :source-runtime:test :translation-runtime:test :backup-runtime:test
...
| 공통 실행 | `source-runtime/`, `translation-runtime/`, `backup-runtime/` | 수집/번역 공급자, ZIP codec과 실행 정책. 양쪽 호출자와 테스트를 함께 검증 |
