...
작성일: 2026-09-05. 상태: 공유 모델 및 웹 클라이언트용 서버 저장·읽기 API 구현.
웹 클라이언트는 `/frontend`에서 별도로 구현하며 Drive 자동 백업은 미구현입니다.
세션 로그인·서재·챕터·진행률·책갈피·번역 백업의 연동 계약은
[서버 웹 API](../server/WEB_API.md)를 참고하세요. 현재 구현 범위와 독립 빌드 명령은
...
| `frontend` (별도 작업) | 웹 서재·카탈로그·리더·백업 설정, 기기 오프라인 저장 |
