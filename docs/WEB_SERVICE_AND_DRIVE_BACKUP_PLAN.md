...
작성일: 2026-09-05. 상태: 설계 제안. 서버, 웹 클라이언트, Drive 자동 백업은 아직 구현되지 않았습니다.
...
| 모듈 | 책임 |
| --- | --- |
| `core-model` | 책·챕터·문단·번역 식별자, 페이지 계약 |
| `core-content` (신규) | 공급자 인터페이스, HTML 파싱, 정규화, 요청 간격 정책 |
| `core-translation` (신규) | 번역 공급자 계약, 묶음 요청, 사전 적용, 캐시 키 |
| `core-backup` (신규) | 백업 manifest, 체크섬, 복원·충돌 정책, 저장소 인터페이스 |
